...
from core.exceptions import TokenError, NotFoundError, AuthenticationError
...
class TokenService:
    ...
    def __init__(self, auth_storage: AuthStorage, uow: UnitOfWork):
...
# ============================================================
    ...
    def _issue_tokens(self, *, session, user_id: int,family_id:str) -> TokenPair:
        ...
        self.auth_storage.create_refresh_token(
            session=session,
            user_id=user_id,
...
# ============================================================
    ...
    def refresh_tokens(self, *, refresh_token: str) -> TokenPair:
        ...
        with self.uow as session:
            ...
            try:
                record = self.auth_storage.get_refresh_token(
                    session=session,
                    token_hash=token_hash
                )
            ...
            if record.used:
                ...
                self.auth_storage.revoke_token_family(
                    session=session,
            ...
            self.auth_storage.mark_refresh_token_used(
                session=session,
                token_id=record.id
            )
            logger.info(
...
# ============================================================
    ...
    def revoke_refresh_token(self, raw_token: str) -> None:
        ...
        with self.uow as session:
            ...
            self.auth_storage.revoke_refresh_token(
                session=session,
                token_id=record.id
            )
    ...
    def logout_all(self, user_id: int) -> None:
        ...
        with self.uow as session:
            ...
            self.auth_storage.revoke_tokens_by_user(
                session=session,
                user_id=user_id
            )
